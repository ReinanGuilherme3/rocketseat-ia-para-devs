...
// centenas de chunks viram vários requests de embeddings, e respostas do chat model
...
builder.AddNpgsqlDataSource("parentrag", configureDataSourceBuilder: dataSourceBuilder =>
{
    dataSourceBuilder.UseVector();
});             // nome igual ao AddDatabase("parentrag") no AppHost
...
builder.Services.AddSingleton<ParentChildChunkStore>();
builder.Services.AddSingleton<ParentRagPipeline>();

var app = builder.Build();

// Passo 1: garante o schema do banco e indexa o livro (só na primeira execução).
var chunkStore = app.Services.GetRequiredService<ParentChildChunkStore>();
...
await ParentDocumentIndexer.IndexIfNeededAsync(
    chunkStore,
    embeddingGenerator,
...
var ragPipeline = app.Services.GetRequiredService<ParentRagPipeline>();

app.MapRagAskEndpoints("Parent RAG - Os Sertões", question => ragPipeline.AnswerAsync(question));

// Mantém o loop de console funcionando também: se você anexar um terminal de
// verdade (ex: "aspire exec --resource parentrag-app -- dotnet run"), continua
